...
def drawFrames(image, vertices, faces, delta, alpha, num_scale, frames_count, green, red, height, width):
    ...
    for i in range(1, frames_count + 1, ):
        ...
        if (I*ALPHA / frames_count) <= (2 * np.pi): #Ошибка была в i*num_scale, но нужно же по градусам
    ...
    for i in range(1, frames_count + 1):
        ...
        newvertices = transform(vertices)
        # трансформируем:
        # Так как мы переопределили позиции Ex, Ey, то произведем обратный порядок умножения
        # ретрансляция
        translates = np.dot(newvertices, np.linalg.inv(translation(delta)))
        # поворот
...
def animation_main():
    ...
    frames_count = 10  # с увеличением фреймов уменьшается чайник. min=6, после выходит за рамки
    num_scale = 2
    delta = [height / 2, width / 2]  #не менять!
    alpha = 2 * np.pi
